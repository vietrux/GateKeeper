...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("data/logs/web_app.log"), logging.StreamHandler()],
)

# Configuration
db_path = "data/databases/car_park.db"
app = Flask(__name__)
...
def dashboard():
    ...
    try:
        ...
        return render_template(
            "dashboard.html", plate_count=plate_count, recent_activity=recent_activity
        )
...
def add_plate():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            cursor.execute(
                "SELECT 1 FROM plates WHERE plate_number = ?", (plate_number,)
            )
            if cursor.fetchone():
            ...
            # Add new plate
            cursor.execute(
                "INSERT INTO plates (plate_number) VALUES (?)", (plate_number,)
            )
