    ...
    def connect_to_database(self):
        ...
        try:
            ...
            if not cursor.fetchone():
                ...
                cursor.execute("CREATE TABLE plates (id INTEGER PRIMARY KEY, plate_number TEXT, owner_name TEXT, access_level INTEGER)")
                self.db_conn.commit()
    ...
    def run(self):
        ...
        try:
                ...
                if self.serial_conn.in_waiting > 0:
                    ...
                    if "CAR_DETECTED" in serial_data:
                        ...
                        if plate_number:
                            ...
                            logger.debug(f"Checking if plate {plate_number} is authorized")
                        ...
                        logger.debug(f"Car detection event processing completed in {event_time:.3f} seconds")
                else:
                    idle_count += 1
            ...
            if self.camera_thread:
                ...
                logger.debug(f"Camera thread terminated: {'Yes' if not self.camera_thread.is_alive() else 'No, had to leave it'}")
            if self.serial_conn:
